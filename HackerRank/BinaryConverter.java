...
class Result {
    ...
    // Write your code here
        int size = 0;
        SinglyLinkedListNode firstNode = binary;
        while (binary.next != null) {
            binary = binary.next;
            size++;
        }
        binary = firstNode;
        long IntegerVersion = 0;
        int i = size;
        do {
            IntegerVersion += binary.data*Math.pow(2, i);
            System.out.println(binary.data + " × 2^" + i);
            binary = binary.next;
            i--;
            System.out.println("sum: " + IntegerVersion);
        } while (binary != null); 
